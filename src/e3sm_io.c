...
int main (int argc, char **argv) {
    ...
    while ((i = getopt (argc, argv, "hkvdnmto:r:WR:s:")) != EOF) switch (i) {
            case 'v':
                verbose = 1;
                break;
            case 'k':
    ...
    if (run_f_case) {
        ...
        if (tst_varn) {
            if (tst_rd) {
                ...
                nerrs +=
                    run_varn_F_case_rd (io_comm, in_dir, outfname, nvars, num_recs, noncontig_buf,
    ...
    if (run_g_case) {
        if (verbose && rank == 0) {
            printf ("number of requests for D1=%d D2=%d D3=%d D4=%d D5=%d D6=%d\n", contig_nreqs[0],
                    contig_nreqs[1], contig_nreqs[2], contig_nreqs[3], contig_nreqs[4],
                    contig_nreqs[5]);
        }
