...
static int write_small_vars_F_case (int ncid,
    ...
    if (rec_no == 0) {
        /* lev */
        err = ncmpi_iput_var_double (ncid, varids[i++], *dbl_buf, NULL);
        ERR
        *dbl_buf += lev + gap;
        /* hyam */
        err = ncmpi_iput_var_double (ncid, varids[i++], *dbl_buf, NULL);
    ...
    /* time */
    start[0] = rec_no;
    err      = ncmpi_iput_var1_double (ncid, varids[i++], start, *dbl_buf, NULL);
    ERR
    *dbl_buf += 1 + gap;
    /* date */
    start[0] = rec_no;
    err      = ncmpi_iput_var1_int (ncid, varids[i++], start, *int_buf, NULL);
    ERR
    *int_buf += 1;
    /* datesec */
    start[0] = rec_no;
    ...
    *int_buf += 1;
    ...
    count[0] = 1;
    count[1] = nbnd;
    err      = ncmpi_iput_vara_double (ncid, varids[i++], start, count, *dbl_buf, NULL);
    ERR
    *dbl_buf += nbnd + gap;
    /* date_written */
    start[0] = rec_no;
    ...
    count[0] = 1;
    count[1] = nchars;
    err      = ncmpi_iput_vara_text (ncid, varids[i++], start, count, *txt_buf, NULL);
    ERR
    *txt_buf += nchars;
    /* time_written */
    start[0] = rec_no;
    ...
    *txt_buf += nchars;
    ...
    if (rec_no == 0) {
        /* ndbase */
        err = ncmpi_iput_var_int (ncid, varids[i++], *int_buf, NULL);
        ERR
        *int_buf += 1;
        /* nsbase */
        err = ncmpi_iput_var_int (ncid, varids[i++], *int_buf, NULL);
...
static int read_small_vars_F_case (int ncid,
                                   int vid, /* starting variable ID */
                                   int *varids,
                                   int rec_no,
                                   int gap,
    ...
    if (rec_no == 0) {
        /* lev */
        err = ncmpi_iget_var_double (ncid, varids[i++], *dbl_buf, NULL);
        ERR
        *dbl_buf += lev + gap;
        /* hyam */
        err = ncmpi_iget_var_double (ncid, varids[i++], *dbl_buf, NULL);
    ...
    /* time */
    start[0] = rec_no;
    err      = ncmpi_iget_var1_double (ncid, varids[i++], start, *dbl_buf, NULL);
    ERR
    *dbl_buf += 1 + gap;
    /* date */
    start[0] = rec_no;
    err      = ncmpi_iget_var1_int (ncid, varids[i++], start, *int_buf, NULL);
    ERR
    *int_buf += 1;
    /* datesec */
    start[0] = rec_no;
    ...
    *int_buf += 1;
    ...
    count[0] = 1;
    count[1] = nbnd;
    err      = ncmpi_iget_vara_double (ncid, varids[i++], start, count, *dbl_buf, NULL);
    ERR
    *dbl_buf += nbnd + gap;
    /* date_written */
    start[0] = rec_no;
    ...
    count[0] = 1;
    count[1] = nchars;
    err      = ncmpi_iget_vara_text (ncid, varids[i++], start, count, *txt_buf, NULL);
    ERR
    *txt_buf += nchars;
    /* time_written */
    start[0] = rec_no;
    ...
    *txt_buf += nchars;
    ...
    if (rec_no == 0) {
        /* ndbase */
        err = ncmpi_iget_var_int (ncid, varids[i++], *int_buf, NULL);
        ERR
        *int_buf += 1;
        /* nsbase */
        err = ncmpi_iget_var_int (ncid, varids[i++], *int_buf, NULL);
...
#define SET_TYPE(kind)                                                                \
    {                                                                                 \
        ...
        err          = ncmpi_inq_varoffset (ncid, varids[i], &var_offset);            \
        ERR                                                                           \
    ...
    int *const blocklens[3]) /* request's block lengths */
{
    ...
    for (i = 0; i < 3; i++)
        for (nelems[i] = 0, j = 0; j < xnreqs[i]; j++) nelems[i] += blocklens[i][j];

    if (verbose && rank == 0) printf ("nelems=%zd %zd %zd\n", nelems[0], nelems[1], nelems[2]);

    /* allocate and initialize write buffer for small variables */
    dbl_buflen = nelems[1] * 2 + nelems[0] + gap * 3 + 3 * dims[2][0] + 3 * (dims[2][0] + 1) + 8 +
                 2 + 20 * gap;
    ...
    if (nvars == 414) {
        /* for h0 file */
        err = def_F_case_h0 (ncid, dims[2], nvars, varids);
        ERR
    } else {
        /* for h1 file */
        err = def_F_case_h1 (ncid, dims[2], nvars, varids);
        ERR
    }
    ...
    /* lat */
    var_types[i] = type[1];
    err          = ncmpi_inq_varoffset (ncid, varids[i], &offset_fix);
    ERR
    var_disps[i]     = 0;
    buf_disps[0]     = 0;
    ...
    err = ncmpi_inq_varoffset (ncid, varids[i], &offset_rec);
    ERR
    ...
    if (noncontig_buf) {
        ...
        MPI_Type_create_hindexed (nvars - 30, buf_blocklens + 30, buf_disps + 30, REC_ITYPE,
                                  &buftype_rec);
    ...
    for (j = 0; j < num_recs; j++) {
        if (j > 0) {
            for (k = 30; k < nvars; k++) var_disps[k] += rec_size;
        }
        /* concatenate nvars-30 var_types[] into filetype_rec[j] */
        MPI_Type_create_struct (nvars - 30, var_blocklens + 30, var_disps + 30, var_types + 30,
                                filetype_rec + j);
    ...
    err = ncmpi_put_vard_all (ncid, varids[0], filetype_dbl, dbl_buf, fix_buflen, buftype_dbl);
    ERR
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        if (rank == 0) {
            ...
            err = write_small_vars_F_case (ncid, i, varids, rec_no, gap, dims[2][0], dims[2][0] + 1,
                                           2, 8, &int_buf_ptr, &txt_buf_ptr, &dbl_buf_ptr);
            ERR
            my_nreqs += 27;
        }
        i += 27;
        ...
        err = ncmpi_put_vard_all (ncid, varids[30], filetype_rec[rec_no], rec_buf, rec_buflen,
                                  buftype_rec);
        ERR
    }
...
#define POST_VARN(k, num, vid)                                                         \
    for (j = 0; j < num; j++) {                                                        \
        err = ncmpi_iput_varn (ncid, vid + j, xnreqs[k - 1], starts_D##k, counts_D##k, \
    ...
    int *int_buf)            /* buffer for int var */
{
    ...
    /* allocate and initialize write buffer for small variables */
    dbl_buflen =
        nelems[1] * 2 + nelems[0] + 3 * dims[2][0] + 3 * (dims[2][0] + 1) + 8 + 2 + 20 * gap;
    if (dbl_bufp != NULL) {
        dbl_buf = dbl_bufp;
    } else {
        dbl_buf = (double *)malloc (dbl_buflen * sizeof (double));
...
#define POST_VARN_RD(k, num, vid)                                                                  \
    for (j = 0; j < num; j++) {                                                                    \
        err = ncmpi_iget_varn (ncid, vid + j, nreqs[k - 1], starts_D##k, counts_D##k, rec_buf_ptr, \
                               -1, REC_ITYPE, NULL);                                               \
        ERR                                                                                        \
        rec_buf_ptr += nelems[k - 1] + gap;                                                        \
        my_nreqs += nreqs[k - 1];                                                                  \
    ...
    int *int_buf)          /* buffer for int var */
{
    ...
    if (nvars == 414) {
        /* for h0 file */
        err = inq_F_case_h0 (ncid, dims[2], nvars, varids);
        ERR
    } else {
        /* for h1 file */
        err = inq_F_case_h1 (ncid, dims[2], nvars, varids);
        ERR
    }
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        if (rank == 0) {
            my_nreqs += 27;
            /* post nonblocking requests using ncmpi_iput_varn() */
            err = read_small_vars_F_case (ncid, i, varids, rec_no, gap, dims[2][0], dims[2][0] + 1,
                                          2, 8, &int_buf_ptr, &txt_buf_ptr, &dbl_buf_ptr);
            ERR
        }
        i += 27;
