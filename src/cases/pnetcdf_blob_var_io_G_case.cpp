...
/*----< blob_var_io_F_case.cpp() >-------------------------------------------*/
int pnetcdf_blob_G_case(e3sm_io_config &cfg,
                        e3sm_io_decom  &decom,
                        e3sm_io_driver &driver)
{
    ...
    MPI_Offset malloc_size, sum_size, m_alloc=0, max_alloc;
    ...
    /* there are num_decomp_vars number of decomosition variables */
...
#define FLUSH_ALL_RECORDS_AT_ONCE
    ...
    if (global_rank == 0) {
        ...
        printf("No. of I/O flush calls             = %d\n", nflushes);
