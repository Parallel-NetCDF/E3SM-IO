...
#define PUT_GATTR_TXT(name, buf) {                                           \
    err = ncmpi_put_att_text(ncid, NC_GLOBAL, name, strlen(buf), buf);       \
    if (err != NC_NOERR) {                                                   \
...
#define PUT_GATTR_INT(name, val) {                                           \
    int buf = val;                                                           \
    err = ncmpi_put_att(ncid, NC_GLOBAL, name, NC_INT, 1, &buf);             \
    if (err != NC_NOERR) {                                                   \
        printf("Error in %s:%d: NC_GLOBAL - %s\n", __FILE__, __LINE__,       \
               ncmpi_strerror(err));                                         \
        goto err_out;                                                        \
...
#define PUT_GATTR_DBL(name, val) {                                           \
    double buf = val;                                                        \
    err = ncmpi_put_att(ncid, NC_GLOBAL, name, NC_DOUBLE, 1, &buf);          \
    if (err != NC_NOERR) {                                                   \
        printf("Error in %s:%d: NC_GLOBAL - %s\n", __FILE__, __LINE__,       \
               ncmpi_strerror(err));                                         \
        goto err_out;                                                        \
...
#define PUT_ATTR_TXT(name, buf) {                                            \
    err = ncmpi_put_att(ncid, *varid, name, NC_CHAR, strlen(buf), (char*)buf);       \
    CHECK_VAR_ERR(*varid)                                                    \
}
...
#define PUT_GATTR_INT(name, val) {                                       \
    int buf = val;                                                       \
    err = driver.put_att(ncid, E3SM_IO_GLOBAL_ATTR, name, MPI_INT, 1, &buf); \
    CHECK_ERR                                                            \
}
#define PUT_GATTR_DBL(name, val) {                                       \
    double buf = val;                                                    \
    err = driver.put_att(ncid, E3SM_IO_GLOBAL_ATTR, name, MPI_DOUBLE, 1, &buf); \
    CHECK_ERR                                                            \
}
#define PUT_ATTR_TXT(name, buf) {                                        \
...
/*----< define_global_attributes() >-----------------------------------------*/
static
int define_global_attributes(e3sm_io_config &cfg,
                             e3sm_io_decom  &decom,
...
/*----< blob_def_G_case() >--------------------------------------------------*/
int blob_def_G_case(e3sm_io_config   &cfg,
                    e3sm_io_decom    &decom,
                    e3sm_io_driver   &driver,
                    int               ncid,         /* file ID */
                    int              *varids)       /* variable IDs */
{
    ...
    err = define_global_attributes(cfg, decom, driver, ncid);
    CHECK_ERR
    ...
    */
    ...
    dimids_D[2][1] = dim_nCells;    dimids_D[2][2] = dim_nVertLevels;
    dimids_D[3][1] = dim_nEdges;    dimids_D[3][2] = dim_nVertLevels;
    ...
    if (cfg.strategy == blob) {
        ...
        fix_D[3][0] = dim_nEdges;    fix_D[3][1] = dim_nVertLevels;
        fix_D[4][0] = dim_nVertices; fix_D[4][1] = dim_nVertLevels;
        fix_D[5][0] = dim_nCells;    fix_D[5][1] = dim_nVertLevelsP1;
        ...
        rec_D[2][1] = dim_nCells;    rec_D[2][2] = dim_nVertLevels;
        rec_D[3][1] = dim_nEdges;    rec_D[3][2] = dim_nVertLevels;
        rec_D[4][1] = dim_nVertices; rec_D[4][2] = dim_nVertLevels;
        rec_D[5][1] = dim_nCells;    rec_D[5][2] = dim_nVertLevelsP1;
    }
    ...
    if (cfg.strategy == blob) {
        for (i=0; i<decom.num_decomp; i++) {
            ...
            DEF_VAR(name, DTYPE_INT, 1, dimids)
            PUT_ATTR_TXT("description", "Number of noncontiguous requests per blob")
            PUT_ATTR_INT("global_dimids", decom.ndims[i], dimids_D[i]+1)
            sprintf(name, "D%d.blob_start", i+1);
            DEF_VAR(name, DTYPE_INT64, 1, dimids)
            ...
            DEF_VAR(name, DTYPE_INT, 2, dimids)
    ...
    DEF_VAR("salinitySurfaceRestoringTendency", DTYPE_DOUBLE, 2, rec_D[0])
    ...
    DEF_VAR("vertTransportVelocityTop", DTYPE_DOUBLE, ndims, rec_D[5])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("vertGMBolusVelocityTop", DTYPE_DOUBLE, ndims, rec_D[5])
    PUT_ATTR_TXT("units", "m s^{-1}")
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("layerThickness", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("normalVelocity", DTYPE_DOUBLE, ndims, rec_D[3])
    PUT_ATTR_TXT("units", "m s^{-1}")
    ...
    DEF_VAR("ssh", DTYPE_DOUBLE, 2, rec_D[0])
    PUT_ATTR_TXT("units", "m")
    PUT_ATTR_TXT("long_name", "sea surface height")
    PUT_ATTR_DECOMP(one, 2, dimids_D[0])

    /* int maxLevelEdgeTop(nEdges) */
    DEF_VAR("maxLevelEdgeTop", DTYPE_INT, 1, fix_D[1])
    ...
    DEF_VAR("vertCoordMovementWeights", DTYPE_DOUBLE, 1, &dim_nVertLevels)
    PUT_ATTR_TXT("units", "unitless")
    ...
    DEF_VAR("edgeMask", DTYPE_INT, ndims, fix_D[3])
    PUT_ATTR_TXT("units", "unitless")
    ...
    ndims = (cfg.strategy == blob) ? 1 : 2;
    DEF_VAR("cellMask", DTYPE_INT, ndims, fix_D[2])
    PUT_ATTR_TXT("units", "unitless")
    ...
    ndims = (cfg.strategy == blob) ? 1 : 2;
    DEF_VAR("vertexMask", DTYPE_INT, ndims, fix_D[4])
    PUT_ATTR_TXT("units", "unitless")
    ...
    DEF_VAR("refZMid", DTYPE_DOUBLE, 1, &dim_nVertLevels)
    PUT_ATTR_TXT("units", "m")
    ...
    DEF_VAR("refLayerThickness", DTYPE_DOUBLE, 1, &dim_nVertLevels)
    PUT_ATTR_TXT("units", "m")
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("relativeVorticityCell", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("relativeVorticity", DTYPE_DOUBLE, ndims, rec_D[4])
    PUT_ATTR_TXT("units", "s^{-1}")
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("divergence", DTYPE_DOUBLE, ndims, rec_D[2])
    PUT_ATTR_TXT("units", "s^{-1}")
    PUT_ATTR_TXT("long_name", "divergence of horizontal velocity")
    PUT_ATTR_DECOMP(three, 3, dimids_D[2])

    /* double areaCellGlobal(Time) */
    DEF_VAR("areaCellGlobal", DTYPE_DOUBLE, 1, &dim_Time)
    ...
    DEF_VAR("areaEdgeGlobal", DTYPE_DOUBLE, 1, &dim_Time)
    PUT_ATTR_TXT("units", "m^2")
    ...
    DEF_VAR("areaTriangleGlobal", DTYPE_DOUBLE, 1, &dim_Time)
    PUT_ATTR_TXT("units", "m^2")
    ...
    DEF_VAR("volumeCellGlobal", DTYPE_DOUBLE, 1, &dim_Time)
    ...
    DEF_VAR("volumeEdgeGlobal", DTYPE_DOUBLE, 1, &dim_Time)
    PUT_ATTR_TXT("units", "m^3")
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("vertVelocityTop", DTYPE_DOUBLE, ndims, rec_D[5])
    PUT_ATTR_TXT("units", "m s^{-1}")
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("velocityZonal", DTYPE_DOUBLE, ndims, rec_D[2])
    PUT_ATTR_TXT("units", "m s^{-1}")
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("velocityMeridional", DTYPE_DOUBLE, ndims, rec_D[2])
    PUT_ATTR_TXT("units", "m s^{-1}")
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("displacedDensity", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("potentialDensity", DTYPE_DOUBLE, ndims, rec_D[2])
    PUT_ATTR_TXT("units", "kg m^{-3}")
    ...
    DEF_VAR("columnIntegratedSpeed", DTYPE_DOUBLE, 2, rec_D[0])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("temperatureHorizontalAdvectionTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("salinityHorizontalAdvectionTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("temperatureVerticalAdvectionTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("salinityVerticalAdvectionTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("temperatureVertMixTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("salinityVertMixTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("temperatureSurfaceFluxTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("salinitySurfaceFluxTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("temperatureShortWaveTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    PUT_ATTR_TXT("units", "degrees Celsius per second")
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("temperatureNonLocalTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("salinityNonLocalTendency", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("temperature", DTYPE_DOUBLE, ndims, rec_D[2])
    ...
    ndims = (cfg.strategy == blob) ? 2 : 3;
    DEF_VAR("salinity", DTYPE_DOUBLE, ndims, rec_D[2])
