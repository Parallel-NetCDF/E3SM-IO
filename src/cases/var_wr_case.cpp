...
#define FIX_VAR_IPUT(varp, dp, itype, buf)                                                         \
    {                                                                                              \
        if (((cfg.strategy == canonical) || (cfg.strategy == log))) {                              \
            ...
            err = driver.put_vara (ncid, varp.vid, itype, &decom.start[dp], &decom.count[dp], buf, \
                                   nb);                                                            \
            my_nreqs++;                                                                            \
            buf += decom.count[dp] + gap;                                                          \
        }                                                                                          \
...
#define REC_VAR_IPUT(varp, dp, itype, buf)                                                  \
    {                                                                                       \
        if (((cfg.strategy == canonical) || (cfg.strategy == log))) {                       \
            ...
            err = driver.put_vara (ncid, varp.vid, itype, starts[dp], counts[dp], buf, nb); \
            my_nreqs++;                                                                     \
            buf += decom.count[dp] + gap;                                                   \
        }                                                                                   \
        CHECK_VAR_ERR (varp.vid)                                                            \
...
int e3sm_io_case::var_wr_case (e3sm_io_config &cfg,
    ...
    if (cfg.api == adios) {
        for (j = 0; j < decom.num_decomp; j++) {
            err = driver.put_varl (ncid, vars[j].vid, MPI_LONG_LONG, decom.raw_offsets[j], nb);
            CHECK_ERR
        }
    ...
    for (rec_no = 0; rec_no < cmeta->nrecs; rec_no++) {
        ...
        if (cfg.comp_time > 0) {
            ...
            tstart = MPI_Wtime ();
            while (MPI_Wtime () - tstart < cfg.comp_time) {
                if (rec_no == 0) {
                    for (j = 0; j < wr_buf.fix_txt_buflen; j++) fix_txt_buf_ptr[j] = 'a' + cfg.rank;
                    for (j = 0; j < wr_buf.fix_int_buflen; j++) fix_int_buf_ptr[j] = cfg.rank;
                    for (j = 0; j < wr_buf.fix_flt_buflen; j++) fix_flt_buf_ptr[j] = cfg.rank;
                    for (j = 0; j < wr_buf.fix_dbl_buflen; j++) fix_dbl_buf_ptr[j] = cfg.rank;
                }
                for (j = 0; j < wr_buf.rec_txt_buflen_itr; j++) rec_txt_buf_ptr[j] = 'a' + cfg.rank;
                for (j = 0; j < wr_buf.rec_int_buflen_itr; j++) rec_int_buf_ptr[j] = cfg.rank;
                for (j = 0; j < wr_buf.rec_flt_buflen_itr; j++) rec_flt_buf_ptr[j] = cfg.rank;
                for (j = 0; j < wr_buf.rec_dbl_buflen_itr; j++) rec_dbl_buf_ptr[j] = cfg.rank;
            }
            cmeta->compute_time += MPI_Wtime () - tstart;
        }
        ...
        if (cfg.api != adios) {
            for (i = 0; i < decom.num_decomp; i++) {
                if (cfg.strategy == blob)
                    ...
                    for (j = 0; j < decom.contig_nreqs[i]; j++) decom.w_starts[i][j][0] = rec_no;
                }
            }
        }
    ...
    if (vars != NULL) {
        for (i = 0; i < nvars; i++)
            if (vars[i]._name != NULL) free (vars[i]._name);
        free (vars);
    }
    ...
    cmeta->end2end_time    = MPI_Wtime () - cmeta->end2end_time - cmeta->compute_time;
    cmeta->post_time -= cmeta->compute_time;
