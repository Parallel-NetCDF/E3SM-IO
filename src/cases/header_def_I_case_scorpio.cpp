...
#define PUT_ATTR_TXT(name, buf) {                                          \
    err = e3sm_io_scorpio_put_att(driver, ncid, varid->vid, name, NC_CHAR, \
                                  strlen (buf), (void *)buf);              \
    CHECK_VAR_ERR (varid->vid)                                             \
}
#define PUT_ATTR_INT1(name, val)  {                                        \
    int buf = val;                                                         \
    err = e3sm_io_scorpio_put_att(driver, ncid, varid->vid, name, NC_INT,  \
                                  1, (void*)(&buf));                       \
    CHECK_VAR_ERR (varid->vid)                                             \
}
...
#define PUT_ATTR_FLOAT(name, num, buf) {                                   \
    float flt = buf;                                                       \
    err = e3sm_io_scorpio_put_att(driver, ncid, varid->vid, name, NC_FLOAT,\
                                  num, (void *)(&flt));                    \
    CHECK_VAR_ERR (varid->vid)                                             \
}
#define PUT_ATTR_INT64(name, num, buf) {                                   \
    err = e3sm_io_scorpio_put_att(driver, ncid, varid->vid, name,          \
               ...
               int             ncid)
{
    ...
    // PIO attributes
#define prefix "/__pio__/"
    PUT_GATTR_INT ("fillmode", 256);

    /* 747 global attributes: */
#undef prefix
#define prefix "pio_global/"
                       ...
                       io_buffers       *wr_buf)
{
    ...
    /* float lon(lon) */
    DEF_VAR("lon", NC_FLOAT, 1, &dim_lon, -1)
    ...
    /* float lat(lat) */
    DEF_VAR("lat", NC_FLOAT, 1, &dim_lat, -1)
    ...
    /* float area(lat, lon) */
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 2;
    DEF_VAR("area", NC_FLOAT, ndims, fix_dimids[0], 0)
    ...
    /* float topo(lat, lon) */
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 2;
    DEF_VAR("topo", NC_FLOAT, ndims, fix_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 2;
    DEF_VAR("landfrac", NC_FLOAT, ndims, fix_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 2;
    DEF_VAR("landmask", NC_INT, ndims, fix_dimids[0], 0)
    ...
    /* int pftmask(lat, lon) */
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 2;
    DEF_VAR("pftmask", NC_INT, ndims, fix_dimids[0], 0)
    ...
    if (cfg.hist == h0) {  /* h0 only */
        ...
        ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 3;
        DEF_VAR("ZSOI", NC_FLOAT, ndims, fix_dimids[1], 1)
        ...
        ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 3;
        DEF_VAR("DZSOI", NC_FLOAT, ndims, fix_dimids[1], 1)
        ...
        ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 3;
        DEF_VAR("WATSAT", NC_FLOAT, ndims, fix_dimids[1], 1)
        ...
        ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 3;
        DEF_VAR("SUCSAT", NC_FLOAT, ndims, fix_dimids[1], 1)
        ...
        ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 3;
        DEF_VAR("BSW", NC_FLOAT, ndims, fix_dimids[1], 1)
        ...
        ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 3;
        DEF_VAR("HKSAT", NC_FLOAT, ndims, fix_dimids[1], 1)
        ...
        ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 3;
        DEF_VAR("ZLAKE", NC_FLOAT, ndims, fix_dimids[2], 2)
        ...
        ndims = (cfg.strategy == blob && cfg.api != adios) ? 1 : 3;
        DEF_VAR("DZLAKE", NC_FLOAT, ndims, fix_dimids[2], 2)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ACTUAL_IMMOB", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ACTUAL_IMMOB_P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ADSORBTION_P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("AGNPP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("AGWDNPP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ALT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ALTMAX", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ALTMAX_LASTYEAR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("AR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("AVAILC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("AVAIL_RETRANSP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("BAF_CROP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("BAF_PEATF", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("BCDEP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("BGNPP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("BIOCHEM_PMIN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("BIOCHEM_PMIN_TO_PLANT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("BTRAN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("BUILDHEAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CH4PROD", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CH4_SURF_AERE_SAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CH4_SURF_AERE_UNSAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CH4_SURF_DIFF_SAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CH4_SURF_DIFF_UNSAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CH4_SURF_EBUL_SAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CH4_SURF_EBUL_UNSAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("COL_PTRUNC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("CONC_CH4_SAT", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("CONC_CH4_UNSAT", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("CONC_O2_SAT", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("CONC_O2_UNSAT", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CPOOL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDC_HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDC_LOSS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDC_TO_LITR2C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDC_TO_LITR3C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("CWDC_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDN_TO_LITR2N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDN_TO_LITR3N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("CWDN_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDP_TO_LITR2P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("CWDP_TO_LITR3P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("CWDP_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DEADCROOTC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DEADCROOTN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DEADCROOTP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DEADSTEMC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DEADSTEMN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DEADSTEMP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DEFICIT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DENIT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DESORPTION_P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DISPVEGC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DISPVEGN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DISPVEGP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DSTDEP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DSTFLXT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DWB", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DWT_CONV_CFLUX_DRIBBLED", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DWT_CONV_CFLUX_GRC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DWT_CONV_NFLUX_GRC", NC_FLOAT, ndims, rec_dimids[0], 0)
    PUT_ATTR_TXT("long_name", "conversion C flux (immediate loss to atm) (0 at all times except first timestep of year)")
    PUT_ATTR_TXT("units", "gN/m^2/s")
    PUT_ATTR_TXT("cell_methods", "time: mean")
    PUT_ATTR_FLOAT(_FillValue, 1, 1.e+36f)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DWT_CONV_PFLUX_GRC", NC_FLOAT, ndims, rec_dimids[0], 0)
    PUT_ATTR_TXT("long_name", "conversion C flux (immediate loss to atm) (0 at all times except first timestep of year)")
    PUT_ATTR_TXT("units", "gP/m^2/s")
    PUT_ATTR_TXT("cell_methods", "time: mean")
    PUT_ATTR_FLOAT(_FillValue, 1, 1.e+36f)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DWT_SLASH_CFLUX", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DWT_SLASH_NFLUX", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("DWT_SLASH_PFLUX", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("EFLX_DYNBAL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("EFLX_GRND_LAKE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("EFLX_LH_TOT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("EFLX_LH_TOT_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("EFLX_LH_TOT_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ELAI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ER", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ERRH2O", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ERRH2OSNO", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ERRSEB", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ERRSOI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ERRSOL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ESAI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FAREA_BURNED", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FCEV", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FCH4", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FCH4TOCO2", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FCH4_DFSAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FCOV", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FCTR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FGEV", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FGR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FGR12", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FGR_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FGR_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FH2OSFC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FINUNDATED", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FINUNDATED_LAG", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FIRA", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FIRA_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FIRA_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FIRE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FIRE_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FIRE_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FLDS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FPG", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FPG_P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FPI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FPI_P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("FPI_P_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    PUT_ATTR_TXT("long_name", "fraction of potential immobilization of phosphorus")
    PUT_ATTR_TXT("units", "proportion")
    PUT_ATTR_TXT("cell_methods", "time: mean")
    PUT_ATTR_FLOAT(_FillValue, 1, 1.e+36f)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("FPI_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    PUT_ATTR_TXT("long_name", "fraction of potential immobilization of nitrogen")
    PUT_ATTR_TXT("units", "proportion")
    PUT_ATTR_TXT("cell_methods", "time: mean")
    PUT_ATTR_FLOAT(_FillValue, 1, 1.e+36f)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FPSN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FPSN_WC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FPSN_WJ", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FPSN_WP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FROOTC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FROOTC_ALLOC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FROOTC_LOSS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FROOTN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FROOTP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FROST_TABLE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSA", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSA_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSA_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSDS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSDSND", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSDSNDLN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSDSNI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSDSVD", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSDSVDLN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSDSVI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSDSVILN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSH", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSH_G", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSH_NODYNLNDUSE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSH_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSH_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSH_V", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSM", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSM_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSM_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSNO", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSNO_EFF", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSRND", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSRNDLN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSRNI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSRVD", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSRVDLN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("FSRVI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("F_CO2_SOIL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("F_CO2_SOIL_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("F_DENIT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("F_DENIT_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    PUT_ATTR_TXT("long_name", "denitrification flux")
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("F_N2O_DENIT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("F_N2O_NIT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("F_NIT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("F_NIT_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    PUT_ATTR_TXT("long_name", "nitrification flux")
    PUT_ATTR_TXT("units", "gN/m^3/s")
    PUT_ATTR_TXT("cell_methods", "time: mean")
    PUT_ATTR_FLOAT(_FillValue, 1, 1.e+36f)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("GC_HEAT1", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("GC_ICE1", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("GC_LIQ1", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("GPP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("GR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("GROSS_NMIN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("GROSS_PMIN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("H2OCAN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("H2OSFC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("H2OSNO", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("H2OSNO_TOP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("H2OSOI", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("HC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("HCSOI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("HEAT_FROM_AC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("HR_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("HTOP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("INT_SNOW", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LABILEP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LABILEP_TO_SECONDP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LABILEP_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LAISHA", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LAISUN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LAKEICEFRAC", NC_FLOAT, ndims, rec_dimids[2], 2)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LAKEICETHICK", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LAND_UPTAKE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LAND_USE_FLUX", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LEAFC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LEAFC_ALLOC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LEAFC_LOSS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LEAFC_TO_LITTER", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LEAFN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LEAFP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LEAF_MR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LFC2", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITFALL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITHR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR1C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR1C_TO_SOIL1C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR1C_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR1N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR1N_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR1N_TO_SOIL1N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR1N_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR1P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR1P_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR1P_TO_SOIL1P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR1P_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR1_HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR2C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR2C_TO_SOIL2C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR2C_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR2N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR2N_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR2N_TO_SOIL2N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR2N_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR2P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR2P_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR2P_TO_SOIL2P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR2P_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR2_HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR3C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR3C_TO_SOIL3C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR3C_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR3N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR3N_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR3N_TO_SOIL3N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR3N_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR3P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR3P_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR3P_TO_SOIL3P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("LITR3P_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITR3_HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITTERC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITTERC_HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LITTERC_LOSS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LIVECROOTC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LIVECROOTN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LIVECROOTP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LIVESTEMC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LIVESTEMN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("LIVESTEMP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("MR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("M_LITR1C_TO_LEACHING", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("M_LITR2C_TO_LEACHING", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("M_LITR3C_TO_LEACHING", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("M_SOIL1C_TO_LEACHING", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("M_SOIL2C_TO_LEACHING", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("M_SOIL3C_TO_LEACHING", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("M_SOIL4C_TO_LEACHING", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NBP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NDEPLOY", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NDEP_TO_SMINN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NEE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NEM", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NEP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NET_NMIN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NET_PMIN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NFIRE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NFIX_TO_SMINN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("NPP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("OCCLP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("OCCLP_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("OCDEP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("O_SCALAR", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PARVEGLN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PBOT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PCH4", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PCO2", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("PCT_LANDUNIT", NC_FLOAT, ndims, rec_dimids[3], 3)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("PCT_NAT_PFT", NC_FLOAT, ndims, rec_dimids[4], 4)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PDEPLOY", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PDEP_TO_SMINP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PFT_FIRE_CLOSS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PFT_FIRE_NLOSS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PLANT_CALLOC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PLANT_NDEMAND", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PLANT_NDEMAND_COL", NC_FLOAT, ndims, rec_dimids[0], 0)
    PUT_ATTR_TXT("long_name", "N flux required to support initial GPP")
    PUT_ATTR_TXT("units", "gN/m^2/s")
    PUT_ATTR_TXT("cell_methods", "time: mean")
    PUT_ATTR_FLOAT(_FillValue, 1, 1.e+36f)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PLANT_PALLOC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PLANT_PDEMAND", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PLANT_PDEMAND_COL", NC_FLOAT, ndims, rec_dimids[0], 0)
    PUT_ATTR_TXT("long_name", "P flux required to support initial GPP")
    PUT_ATTR_TXT("units", "gN/m^2/s")
    PUT_ATTR_TXT("cell_methods", "time: mean")
    PUT_ATTR_FLOAT(_FillValue, 1, 1.e+36f)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("POTENTIAL_IMMOB", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("POTENTIAL_IMMOB_P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("POT_F_DENIT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("POT_F_NIT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PRIMP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PRIMP_TO_LABILEP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("PRIMP_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PROD1P_LOSS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PSNSHA", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PSNSHADE_TO_CPOOL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PSNSUN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("PSNSUN_TO_CPOOL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("Q2M", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QBOT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QCHARGE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QDRAI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QDRAI_PERCH", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QDRAI_XS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QDRIP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QFLOOD", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QFLX_ICE_DYNBAL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QFLX_LIQ_DYNBAL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QH2OSFC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QINFL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QINTR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QIRRIG_GRND", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QIRRIG_ORIG", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QIRRIG_REAL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QIRRIG_SURF", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QIRRIG_WM", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QOVER", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QOVER_LAG", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QRGWL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QRUNOFF", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QRUNOFF_NODYNLNDUSE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QRUNOFF_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QRUNOFF_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QSNOMELT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QSNWCPICE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QSNWCPICE_NODYNLNDUSE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QSOIL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QVEGE", NC_FLOAT, ndims, rec_dimids[0], 0)
    PUT_ATTR_TXT("long_name", "canopy evaporation")
    PUT_ATTR_TXT("units", "mm/s")
    PUT_ATTR_TXT("cell_methods", "time: mean")
    PUT_ATTR_FLOAT(_FillValue, 1, 1.e+36f)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("QVEGT", NC_FLOAT, ndims, rec_dimids[0], 0)
    PUT_ATTR_TXT("long_name", "canopy transpiration")
    PUT_ATTR_TXT("units", "mm/s")
    PUT_ATTR_TXT("cell_methods", "time: mean")
    PUT_ATTR_FLOAT(_FillValue, 1, 1.e+36f)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("RAIN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("RETRANSN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("RETRANSN_TO_NPOOL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("RETRANSP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("RETRANSP_TO_PPOOL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("RH2M", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("RH2M_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("RH2M_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("RR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SABG", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SABG_PEN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SABV", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SCALARAVG_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SECONDP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SECONDP_TO_LABILEP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SECONDP_TO_OCCLP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SECONDP_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SEEDC_GRC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINN_TO_NPOOL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINN_TO_PLANT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINN_TO_SOIL1N_L1", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINN_TO_SOIL2N_L2", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINN_TO_SOIL2N_S1", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINN_TO_SOIL3N_L3", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINN_TO_SOIL3N_S2", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINN_TO_SOIL4N_S3", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINP_LEACHED", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINP_TO_PLANT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINP_TO_PPOOL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINP_TO_SOIL1P_L1", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINP_TO_SOIL2P_L2", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINP_TO_SOIL2P_S1", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINP_TO_SOIL3P_L3", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINP_TO_SOIL3P_S2", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMINP_TO_SOIL4P_S3", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SMINP_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMIN_NH4", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SMIN_NH4_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMIN_NO3", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMIN_NO3_LEACHED", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SMIN_NO3_RUNOFF", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SMIN_NO3_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOBCMCL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOBCMSL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNODSTMCL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNODSTMSL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOINTABS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOOCMCL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOOCMSL", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOW", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOWDP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOWICE", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOWLIQ", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOW_DEPTH", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOW_SINKS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SNOW_SOURCES", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL1C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL1C_TO_SOIL2C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL1C_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL1N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL1N_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL1N_TO_SOIL2N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL1N_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL1P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL1P_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL1P_TO_SOIL2P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL1P_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL1_HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL2C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL2C_TO_SOIL3C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL2C_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL2N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL2N_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL2N_TO_SOIL3N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL2N_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL2P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL2P_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL2P_TO_SOIL3P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL2P_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL2_HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL3C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL3C_TO_SOIL4C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL3C_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL3N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL3N_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL3N_TO_SOIL4N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL3N_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL3P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL3P_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL3P_TO_SOIL4P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL3P_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL3_HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL4C", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL4C_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL4N", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL4N_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL4N_TO_SMINN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL4N_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL4P", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL4P_TNDNCY_VERT_TRANS", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL4P_TO_SMINP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOIL4P_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOIL4_HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOILC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOILC_HR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOILC_LOSS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOILICE", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOILICE_ICE", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOILLIQ", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOILLIQ_ICE", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOILPSI", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOILWATER_10CM", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOLUTIONP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("SOLUTIONP_vr", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOMHR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SOM_C_LEACHED", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("STORVEGC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("STORVEGN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("STORVEGP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SUPPLEMENT_TO_SMINN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SUPPLEMENT_TO_SMINP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SUPPLY", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SoilAlpha", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("SoilAlpha_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TAUX", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TAUY", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TBOT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TBUILD", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TCS_MONTH_BEGIN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TCS_MONTH_END", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TG", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TG_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TG_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TH2OSFC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("THBOT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TKE1", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TLAI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("TLAKE", NC_FLOAT, ndims, rec_dimids[2], 2)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTCOLC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTCOLCH4", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTCOLN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTCOLP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTECOSYSC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTECOSYSN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTECOSYSP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTLITC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTLITC_1m", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTLITN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTLITP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTLITP_1m", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTPFTC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTPFTN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTPFTP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTSOMC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTSOMC_1m", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTSOMN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTSOMP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTSOMP_1m", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTVEGC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTVEGC_ABG", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTVEGN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TOTVEGP", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TREFMNAV", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TREFMNAV_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TREFMNAV_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TREFMXAV", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TREFMXAV_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TREFMXAV_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TSA", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TSAI", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TSA_R", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TSA_U", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("TSOI", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TSOI_10CM", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("TSOI_ICE", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TV", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TWS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TWS_MONTH_BEGIN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("TWS_MONTH_END", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("T_SCALAR", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("U10", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("URBAN_AC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("URBAN_HEAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("VOLR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("VOLRMCH", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("WA", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("WASTEHEAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("WF", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("WIND", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("WOODC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("WOODC_ALLOC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("WOODC_LOSS", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("WOOD_HARVESTC", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("WOOD_HARVESTN", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("WTGQ", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("W_SCALAR", NC_FLOAT, ndims, rec_dimids[5], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("XR", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("XSMRPOOL", NC_FLOAT, ndims, rec_dimids[0], 0)
    PUT_ATTR_TXT("long_name", "temporary photosynthate C pool")
    PUT_ATTR_TXT("units", "gC/m^2")
    PUT_ATTR_TXT("cell_methods", "time: mean")
    PUT_ATTR_FLOAT(_FillValue, 1, 1.e+36f)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ZBOT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ZWT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ZWT_CH4_UNSAT", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("ZWT_PERCH", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("cn_scalar", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("cp_scalar", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("leaf_npimbalance", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("nlim_m", NC_FLOAT, ndims, rec_dimids[0], 0)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 4;
    DEF_VAR("o2_decomp_depth_unsat", NC_FLOAT, ndims, rec_dimids[1], 1)
    ...
    ndims = (cfg.strategy == blob && cfg.api != adios) ? 2 : 3;
    DEF_VAR("plim_m", NC_FLOAT, ndims, rec_dimids[0], 0)
