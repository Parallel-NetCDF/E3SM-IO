...
#define PUT_GATTR_INT1(name, val) {                                          \
    int buf = val;                                                           \
...
#define PUT_ATTR_INT1(name, val) {                                           \
    int buf = val;                                                           \
    err = driver.put_att(ncid, varid, name, NC_INT, 1, &buf);                \
    CHECK_VAR_ERR(varid)                                                     \
}
...
#define PUT_ATTR_FLOAT(name, num, buf) {                                     \
    float flt = buf;                                                         \
    err = driver.put_att(ncid, varid, name, NC_FLOAT, num, &flt);            \
    CHECK_VAR_ERR(varid)                                                     \
}
               ...
               int             ncid)
{
    ...
    if (cfg.strategy == blob && (cfg.api == pnetcdf || cfg.api == hdf5)) {
        ...
        PUT_GATTR_INT1("global_nprocs", nprocs)
        PUT_GATTR_INT1("num_decompositions", decom.num_decomp)
        PUT_GATTR_INT1("num_subfiles", cfg.num_subfiles)
    }
