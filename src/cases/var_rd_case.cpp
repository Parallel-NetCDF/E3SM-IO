...
#define IGET_VARA_NOADV(varp, itype, buf) {                                   \
                              ...
                              case_meta      *cmeta)
{
    ...
    if (cfg.strategy == blob) {
        ...
        for (j=0; j<decom.num_decomp; j++) {
            ...
            IGET_VARA_NOADV(vars[i], MPI_INT, contig_nreqs+j)
            i++;

            /* write to D*.blob_start, 1D array */
            blob_start[j] = decom.start[j];
            IGET_VARA_NOADV(vars[i], MPI_LONG_LONG, blob_start+j)
            i++;

            /* write to D*.blob_count, 1D array */
            blob_count[j] = decom.count[j];
            IGET_VARA_NOADV(vars[i], MPI_LONG_LONG, blob_count+j);
            i++;
            ...
            IGET_VARA_NOADV(vars[i], MPI_INT, decom.blocklens[j])
            i++;
