...
#define INQ_GET_SIZE(size) {                                                  \
    err = driver.inq_get_size(&size);                                         \
    CHECK_ERR                                                                 \
}
...
#define IGET_VARA_NOADV(itype, buf) {                                         \
    err = driver.get_vara(ncid, varid, itype, start, count, buf, nb);         \
    CHECK_VAR_ERR(varid)                                                      \
    my_nreqs++;                                                               \
    varid++;                                                                  \
}

#define IGET_VARA(varp, itype, adv, buf) {                                    \
    if (cfg.api == adios)                                                     \
        err = scorpio_write_var(driver, rec_no, ncid, varp, itype, buf, nb);  \
    else                                                                      \
        err = driver.get_vara(ncid, varp.vid, itype, start, count, buf, nb);  \
    CHECK_VAR_ERR(varp.vid)                                                   \
    buf += (adv);                                                             \
    my_nreqs++;                                                               \
}

#define IGET_VAR(varp, itype, adv, buf) {                                     \
    if (cfg.api == adios)                                                     \
        err = scorpio_write_var(driver, -1, ncid, varp, itype, buf, nb);      \
    else                                                                      \
        err = driver.get_vara(ncid, varp.vid, itype, NULL, NULL, buf, nb);    \
    CHECK_VAR_ERR(varp.vid)                                                   \
    buf += (adv);                                                             \
    my_nreqs++;                                                               \
}
...
#define FIX_VAR_IGET(varp, dp, itype, buf) {                                  \
/* printf("FIX varp.vid=%d ndims=%d dp=%d nreqs=%d itype=%d name=%s\n",varp.vid,varp.ndims,dp, decom.contig_nreqs[dp],itype,varp._name); */ \
    if (((cfg.strategy == canonical) || (cfg.strategy == log))) {             \
        err = driver.get_varn(ncid, varp.vid, itype, decom.contig_nreqs[dp],  \
        ...
        err = driver.get_vara(ncid, varp.vid, itype, &decom.start[dp],        \
...
#define REC_VAR_IGET(varp, dp, itype, buf) {                                  \
/* printf("REC varp.vid=%d ndims=%d dp=%d nreqs=%d itype=%d name=%s\n",varp.vid,varp.ndims,dp, decom.contig_nreqs[dp],itype,varp._name); */ \
    if (((cfg.strategy == canonical) || (cfg.strategy == log))) {             \
        ...
        err = driver.get_vara(ncid, varp.vid, itype, starts[dp], counts[dp],  \
                              buf, nb);                                       \
        my_nreqs++;                                                           \
        buf += decom.count[dp] + gap;                                         \
    }                                                                         \
...
/*----< var_wr_case() >------------------------------------------------------*/
int e3sm_io_case::var_rd_case(e3sm_io_config &cfg,
                              e3sm_io_decom  &decom,
                              ...
                              case_meta      *cmeta)
{
    ...
    /* create the outget file */
    FILE_OPEN(cmeta->outfile)
    ...
    if (cfg.run_case == F)
        err = inq_F_case(cfg, decom, driver, ncid);
    else if (cfg.run_case == G)
        err = inq_G_case(cfg, decom, driver, ncid);
    else if (cfg.run_case == I)
        err = inq_I_case(cfg, decom, driver, ncid);
    CHECK_ERR
    ...
    if (cfg.strategy == blob) {
        ...
        for (j=0; j<decom.num_decomp; j++) {
            ...
            IGET_VARA_NOADV(MPI_INT, contig_nreqs+j)

            /* write to D*.blob_start, 1D array */
            blob_start[j] = decom.start[j];
            IGET_VARA_NOADV(MPI_LONG_LONG, blob_start+j)

            /* write to D*.blob_count, 1D array */
            blob_count[j] = decom.count[j];
            IGET_VARA_NOADV(MPI_LONG_LONG, blob_count+j);

            /* write to D*.offsets, 2D array */
            IGET_VARA_NOADV(MPI_INT, decom.disps[j])

            /* write to D*.lengths, 2D array */
            IGET_VARA_NOADV(MPI_INT, decom.blocklens[j])
        ...
        for (j=num_decomp_vars; j<nvars; j++) {
            ...
            if (vars[j].decomp_id >= 0) { /* this variable is partitioned */
                if (vars[j].isRecVar) { /* this is a record variable */
                    if (itype == MPI_DOUBLE)
                        REC_VAR_IGET(vars[j], dp, itype, rec_dbl_buf_ptr)
                    else if (itype == MPI_INT)
                        REC_VAR_IGET(vars[j], dp, itype, rec_int_buf_ptr)
                    else if (itype == MPI_CHAR)
                        REC_VAR_IGET(vars[j], dp, itype, rec_txt_buf_ptr)
                    else if (itype == MPI_FLOAT)
                        REC_VAR_IGET(vars[j], dp, itype, rec_flt_buf_ptr)
                }
                else if (rec_no == 0) { /* this is a fixed-size variable */
                    if (itype == MPI_DOUBLE)
                        FIX_VAR_IGET(vars[j], dp, itype, fix_dbl_buf_ptr)
                    else if (itype == MPI_INT)
                        FIX_VAR_IGET(vars[j], dp, itype, fix_int_buf_ptr)
                    else if (itype == MPI_CHAR)
                        FIX_VAR_IGET(vars[j], dp, itype, fix_txt_buf_ptr)
                    else if (itype == MPI_FLOAT)
                        FIX_VAR_IGET(vars[j], dp, itype, fix_flt_buf_ptr)
                }
            }
            else if (sub_rank == 0) {
                ...
                if (vars[j].isRecVar) { /* this is a record variable */
                    ...
                    count[1] = vars[j].vlen;
                    if (itype == MPI_DOUBLE)
                        IGET_VARA(vars[j], itype, adv, rec_dbl_buf_ptr)
                    else if (itype == MPI_INT)
                        IGET_VARA(vars[j], itype, adv, rec_int_buf_ptr)
                    else if (itype == MPI_CHAR)
                        IGET_VARA(vars[j], itype, adv, rec_txt_buf_ptr)
                    else if (itype == MPI_FLOAT)
                        IGET_VARA(vars[j], itype, adv, rec_flt_buf_ptr)
                }
                else if (rec_no == 0) { /* this is a fixed-size variable */
                    if (itype == MPI_DOUBLE)
                        IGET_VAR(vars[j], itype, adv, fix_dbl_buf_ptr)
                    else if (itype == MPI_INT)
                        IGET_VAR(vars[j], itype, adv, fix_int_buf_ptr)
                    else if (itype == MPI_CHAR)
                        IGET_VAR(vars[j], itype, adv, fix_txt_buf_ptr)
                    else if (itype == MPI_FLOAT)
                        IGET_VAR(vars[j], itype, adv, fix_flt_buf_ptr)
                }
            }
        }

        /* flush out all the pending iget requests. Note flush frequency does
