 ...
 *
 * Arguments:
 *     infname:      (IN)  input file name, a NetCDF file containing the data
 *                         decomposition generated by PIO library from an E3SM
 *                         run.
 *     num_decomp    (OUT) number of decompositions
 *     dims:         (OUT) global array dimension lengths of each decomposition.
        ...
        */
        ...
        if (verbose)
            printf ("D%d rank %d: proc_start=%d proc_count=%d start%lld count=%lld\n",
                    decomp_id + 1, rank, proc_start, proc_count, start, count);
