...
/*----< read_io_decomp() >-------------------------------------------------*/
/* Read I/O decomposition file, cfg->cfgpath. The contents of the file are, for
 ...
 *
 * Arguments:
 *   cfg->cfgpath:     (IN)  input file name, a NetCDF file containing the data
                ...
                e3sm_io_decom  *decom)
{
    ...
    err = ncmpi_open(cfg->io_comm, cfg->cfgpath, NC_NOWRITE, info, &ncid);
    CHECK_ERR
    ...
    for (id = 0; id < decom->num_decomp; id++) {
        ...
        err = ncmpi_get_att_longlong(ncid, NC_GLOBAL, name, decom->dims[id]);
        CHECK_ERR
        ...
        err = ncmpi_inq_varid(ncid, name, &varid);
        CHECK_ERR
        err = ncmpi_get_vara_int_all(ncid, varid, &start, &count,
                                     decom->disps[id]);
        CHECK_ERR
        ...
        if (cfg->verbose) {
            ...
            printf("D%d rank %d nreqs=%d contig nreqs=%4d max_blocklen=%d min_blocklen=%d\n",
                    id + 1, rank, nreqs, decom->contig_nreqs[id], max_blocklen,
                    min_blocklen);
        }
    }
