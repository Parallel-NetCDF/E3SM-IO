    ...
    double *D1_fix_dbl_bufp) /* D1 fix double buffer */
{
    ...
    my_nreqs                = 0;
    for (i = 0; i < 6; i++) {
        for (nelems[i] = 0, k = 0; k < xnreqs[i]; k++) nelems[i] += blocklens[i][k];
    }
    ...
    if (xnreqs[2] > 0) {
        FIX_2D_VAR_STARTS_COUNTS (fix_starts_D3, fix_counts_D3, xnreqs[2], disps[2], blocklens[2],
                                  dims[2][1])
    ...
    if (xnreqs[3] > 0) {
        FIX_2D_VAR_STARTS_COUNTS (fix_starts_D4, fix_counts_D4, xnreqs[3], disps[3], blocklens[3],
                                  dims[3][1])
        REC_3D_VAR_STARTS_COUNTS (0, starts_D4, counts_D4, xnreqs[3], disps[3], blocklens[3],
                                  dims[3][1])
    } else {
        fix_starts_D4 = NULL;
        fix_counts_D4 = NULL;
    ...
    if (xnreqs[4] > 0) {
        FIX_2D_VAR_STARTS_COUNTS (fix_starts_D5, fix_counts_D5, xnreqs[4], disps[4], blocklens[4],
                                  dims[4][1])
        REC_3D_VAR_STARTS_COUNTS (0, starts_D5, counts_D5, xnreqs[4], disps[4], blocklens[4],
                                  dims[4][1])
    } else {
        fix_starts_D5 = NULL;
        fix_counts_D5 = NULL;
    ...
    if (xnreqs[5] > 0) {
        REC_3D_VAR_STARTS_COUNTS (0, starts_D6, counts_D6, xnreqs[5], disps[5], blocklens[5],
                                  dims[5][1])
    } else {
        starts_D6 = NULL;
        counts_D6 = NULL;
    ...
    err = def_G_case_h0 (ncid, dims[0], dims[1], dims[2], dims[3], dims[4], dims[5], nvars, varids);
    ERR
    ...
    err = ncmpi_iput_varn (ncid, 8, xnreqs[1], fix_starts_D2, fix_counts_D2, D2_fix_int_buf,
    ...
    err = ncmpi_iput_varn (ncid, 10, xnreqs[3], fix_starts_D4, fix_counts_D4, D4_fix_int_buf,
    ...
    err = ncmpi_iput_varn (ncid, 11, xnreqs[2], fix_starts_D3, fix_counts_D3, D3_fix_int_buf,
    ...
    err = ncmpi_iput_varn (ncid, 12, xnreqs[4], fix_starts_D5, fix_counts_D5, D5_fix_int_buf,
    ...
    err = ncmpi_iput_varn (ncid, 35, xnreqs[0], fix_starts_D1, fix_counts_D1, D1_fix_dbl_buf,
    ...
    err = ncmpi_iput_varn (ncid, 36, xnreqs[0], fix_starts_D1, fix_counts_D1, D1_fix_int_buf,
    ...
    if (rank == 0) {
        ...
        err = ncmpi_bput_vars_double (ncid, 9, start, count, stride, dummy_double_buf, NULL);
        ERR

        /* double refZMid(nVertLevels) */
        err = ncmpi_bput_vars_double (ncid, 13, start, count, stride, dummy_double_buf, NULL);
        ERR

        /* double refLayerThickness(nVertLevels) */
        err = ncmpi_bput_vars_double (ncid, 14, start, count, stride, dummy_double_buf, NULL);
        ERR

        /* double refBottomDepth(nVertLevels) */
        err = ncmpi_bput_vars_double (ncid, 33, start, count, stride, dummy_double_buf, NULL);
        ERR
        ...
        for (rec_no = 0; rec_no < num_recs; rec_no++) {
            ...
            err = ncmpi_bput_vars_text (ncid, 15, start, count, stride, dummy_char_buf, NULL);
            ERR

            /* double areaCellGlobal(Time) */
            err = ncmpi_bput_vars_double (ncid, 20, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double areaEdgeGlobal(Time) */
            err = ncmpi_bput_vars_double (ncid, 21, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double areaTriangleGlobal(Time) */
            err = ncmpi_bput_vars_double (ncid, 22, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double volumeCellGlobal(Time) */
            err = ncmpi_bput_vars_double (ncid, 23, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double volumeEdgeGlobal(Time) */
            err = ncmpi_bput_vars_double (ncid, 24, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double CFLNumberGlobal(Time) */
            err = ncmpi_bput_vars_double (ncid, 25, start, count, stride, dummy_double_buf, NULL);
            ERR
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        for (j = 0; j < nD1_rec_2d_vars; j++) {
            err = ncmpi_iput_varn (ncid, D1_rec_2d_varids[j], xnreqs[0], starts_D1, counts_D1,
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        for (j = 0; j < nD6_rec_3d_vars; j++) {
            err = ncmpi_iput_varn (ncid, D6_rec_3d_varids[j], xnreqs[5], starts_D6, counts_D6,
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        for (j = 0; j < nD3_rec_3d_vars; j++) {
            err = ncmpi_iput_varn (ncid, D3_rec_3d_varids[j], xnreqs[2], starts_D3, counts_D3,
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        for (j = 0; j < nD4_rec_3d_vars; j++) {
            err = ncmpi_iput_varn (ncid, D4_rec_3d_varids[j], xnreqs[3], starts_D4, counts_D4,
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        for (j = 0; j < nD5_rec_3d_vars; j++) {
            err = ncmpi_iput_varn (ncid, D5_rec_3d_varids[j], xnreqs[4], starts_D5, counts_D5,
    ...
    double **D1_fix_dbl_bufp) /* D1 fix double buffer */
{
    ...
    my_nreqs                = 0;
    for (i = 0; i < 6; i++) {
        for (nelems[i] = 0, k = 0; k < nreqs[i]; k++) nelems[i] += blocklens[i][k];
    }
    ...
    if (nreqs[2] > 0) {
        FIX_2D_VAR_STARTS_COUNTS (fix_starts_D3, fix_counts_D3, nreqs[2], disps[2], blocklens[2],
                                  dims[2][1])
        REC_3D_VAR_STARTS_COUNTS (0, starts_D3, counts_D3, nreqs[2], disps[2], blocklens[2],
                                  dims[2][1])
    } else {
        fix_starts_D3 = NULL;
        fix_counts_D3 = NULL;
    ...
    if (nreqs[3] > 0) {
        FIX_2D_VAR_STARTS_COUNTS (fix_starts_D4, fix_counts_D4, nreqs[3], disps[3], blocklens[3],
                                  dims[3][1])
        REC_3D_VAR_STARTS_COUNTS (0, starts_D4, counts_D4, nreqs[3], disps[3], blocklens[3],
                                  dims[3][1])
    } else {
        fix_starts_D4 = NULL;
        fix_counts_D4 = NULL;
    ...
    if (nreqs[4] > 0) {
        FIX_2D_VAR_STARTS_COUNTS (fix_starts_D5, fix_counts_D5, nreqs[4], disps[4], blocklens[4],
                                  dims[4][1])
        REC_3D_VAR_STARTS_COUNTS (0, starts_D5, counts_D5, nreqs[4], disps[4], blocklens[4],
                                  dims[4][1])
    } else {
        fix_starts_D5 = NULL;
        fix_counts_D5 = NULL;
    ...
    if (nreqs[5] > 0) {
        REC_3D_VAR_STARTS_COUNTS (0, starts_D6, counts_D6, nreqs[5], disps[5], blocklens[5],
                                  dims[5][1])
    } else {
        starts_D6 = NULL;
        counts_D6 = NULL;
    ...
    /* define dimensions, variables, and attributes */
    err = inq_G_case_h0 (ncid, dims[0], dims[1], dims[2], dims[3], dims[4], dims[5], nvars, varids);
    ERR
    ...
    /* int maxLevelEdgeTop(nEdges) */
    err = ncmpi_iget_varn (ncid, 8, nreqs[1], fix_starts_D2, fix_counts_D2, D2_fix_int_buf,
                           nelems[1], MPI_INT, NULL);
    ERR
    my_nreqs += nreqs[1];
    ...
    /* int edgeMask(nEdges, nVertLevels) */
    err = ncmpi_iget_varn (ncid, 10, nreqs[3], fix_starts_D4, fix_counts_D4, D4_fix_int_buf,
                           nelems[3], MPI_INT, NULL);
    ERR
    my_nreqs += nreqs[3];

    /* int cellMask(nCells, nVertLevels) */
    err = ncmpi_iget_varn (ncid, 11, nreqs[2], fix_starts_D3, fix_counts_D3, D3_fix_int_buf,
                           nelems[2], MPI_INT, NULL);
    ERR
    my_nreqs += nreqs[2];

    /* int vertexMask(nVertices, nVertLevels) */
    err = ncmpi_iget_varn (ncid, 12, nreqs[4], fix_starts_D5, fix_counts_D5, D5_fix_int_buf,
                           nelems[4], MPI_INT, NULL);
    ERR
    my_nreqs += nreqs[4];

    /* double bottomDepth(nCells)  */
    err = ncmpi_iget_varn (ncid, 35, nreqs[0], fix_starts_D1, fix_counts_D1, D1_fix_dbl_buf,
                           nelems[0], MPI_DOUBLE, NULL);
    ERR
    my_nreqs += nreqs[0];

    /* int maxLevelCell(nCells) */
    err = ncmpi_iget_varn (ncid, 36, nreqs[0], fix_starts_D1, fix_counts_D1, D1_fix_int_buf,
                           nelems[0], MPI_INT, NULL);
    ERR
    my_nreqs += nreqs[0];
    ...
    if (rank == 0) {
        ...
        /* double vertCoordMovementWeights(nVertLevels) */
        err = ncmpi_iget_vars_double (ncid, 9, start, count, stride, dummy_double_buf, NULL);
        ERR

        /* double refZMid(nVertLevels) */
        err = ncmpi_iget_vars_double (ncid, 13, start, count, stride, dummy_double_buf, NULL);
        ERR

        /* double refLayerThickness(nVertLevels) */
        err = ncmpi_iget_vars_double (ncid, 14, start, count, stride, dummy_double_buf, NULL);
        ERR

        /* double refBottomDepth(nVertLevels) */
        err = ncmpi_iget_vars_double (ncid, 33, start, count, stride, dummy_double_buf, NULL);
        ERR
        ...
        for (rec_no = 0; rec_no < num_recs; rec_no++) {
            ...
            /* char xtime(Time, StrLen) */
            err = ncmpi_iget_vars_text (ncid, 15, start, count, stride, dummy_char_buf, NULL);
            ERR

            /* double areaCellGlobal(Time) */
            err = ncmpi_iget_vars_double (ncid, 20, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double areaEdgeGlobal(Time) */
            err = ncmpi_iget_vars_double (ncid, 21, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double areaTriangleGlobal(Time) */
            err = ncmpi_iget_vars_double (ncid, 22, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double volumeCellGlobal(Time) */
            err = ncmpi_iget_vars_double (ncid, 23, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double volumeEdgeGlobal(Time) */
            err = ncmpi_iget_vars_double (ncid, 24, start, count, stride, dummy_double_buf, NULL);
            ERR

            /* double CFLNumberGlobal(Time) */
            err = ncmpi_iget_vars_double (ncid, 25, start, count, stride, dummy_double_buf, NULL);
            ERR
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D1_rec_dbl_buf;
        for (j = 0; j < nD1_rec_2d_vars; j++) {
            err = ncmpi_iget_varn (ncid, D1_rec_2d_varids[j], nreqs[0], starts_D1, counts_D1,
                                   rec_buf_ptr, nelems[0], MPI_DOUBLE, NULL);
            ERR
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D6_rec_dbl_buf;
        for (j = 0; j < nD6_rec_3d_vars; j++) {
            err = ncmpi_iget_varn (ncid, D6_rec_3d_varids[j], nreqs[5], starts_D6, counts_D6,
                                   rec_buf_ptr, nelems[5], MPI_DOUBLE, NULL);
            ERR
            rec_buf_ptr += nelems[5];
            my_nreqs += nreqs[5];
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D3_rec_dbl_buf;
        for (j = 0; j < nD3_rec_3d_vars; j++) {
            err = ncmpi_iget_varn (ncid, D3_rec_3d_varids[j], nreqs[2], starts_D3, counts_D3,
                                   rec_buf_ptr, nelems[2], MPI_DOUBLE, NULL);
            ERR
            rec_buf_ptr += nelems[2];
            my_nreqs += nreqs[2];
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D4_rec_dbl_buf;
        for (j = 0; j < nD4_rec_3d_vars; j++) {
            err = ncmpi_iget_varn (ncid, D4_rec_3d_varids[j], nreqs[3], starts_D4, counts_D4,
                                   rec_buf_ptr, nelems[3], MPI_DOUBLE, NULL);
            ERR
            rec_buf_ptr += nelems[3];
            my_nreqs += nreqs[3];
    ...
    for (rec_no = 0; rec_no < num_recs; rec_no++) {
        ...
        rec_buf_ptr = D5_rec_dbl_buf;
        for (j = 0; j < nD5_rec_3d_vars; j++) {
            err = ncmpi_iget_varn (ncid, D5_rec_3d_varids[j], nreqs[4], starts_D5, counts_D5,
                                   rec_buf_ptr, nelems[4], MPI_DOUBLE, NULL);
            ERR
            rec_buf_ptr += nelems[4];
            my_nreqs += nreqs[4];
