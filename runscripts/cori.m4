                ...
                do
                    ...
                    do
                        ...
                        do
                            ...
                            if [ "$OP" = "w" ] ; then
                                srun -n ${NP} -t ${RTL} -c 4 --cpu_bind=cores /tmp/${USER}_${APP} -g ${NG} -k -o ${OUTDIR}/testfile -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p -y ${FFREQ} ${CONFIG} 
                            else
                                srun -n ${NP} -t ${RTL} -c 4 --cpu_bind=cores /tmp/${USER}_${APP} -g ${NG} -k -i ${OUTDIR}/testfile_rd -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p -y ${FFREQ} ${CONFIG} 
                            fi
