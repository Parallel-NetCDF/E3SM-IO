...
APPS=(../src/e3sm_io.cray ../src/e3sm_io.qiao)
HXS=(-1)
...
do
    ...
    do
        for APP in ${APPS[@]}
        do
            sbcast -v ${APP} /tmp/${USER}_e3sm_io.exe
                ...
                do
                    ...
                    do
                        ...
                        do
                            ...
                            if [ "$OP" = "w" ] ; then
                                echo "srun -n ${NP} -t ${RTL} -c 4 --cpu_bind=cores  /tmp/${USER}_e3sm_io.exe -g ${NG} -k -o ${OUTDIR}/testfile -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p -y ${FFREQ} ${CONFIG}"
                               # srun -n ${NP} -t ${RTL} --cpu_bind=cores /tmp/${USER}_e3sm_io.exe -g ${NG} -k -o ${OUTDIR}/testfile -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p -y ${FFREQ} ${CONFIG}
                            else
                                echo "srun -n ${NP} -t ${RTL} -c 4 --cpu_bind=cores  /tmp/${USER}_e3sm_io.exe -g ${NG} -k -i ${OUTDIR}/testfile_rd -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p -y ${FFREQ} ${CONFIG}"
                                #srun -n ${NP} -t ${RTL} --cpu_bind=cores /tmp/${USER}_e3sm_io.exe -g ${NG} -k -i ${OUTDIR}/testfile_rd -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p -y ${FFREQ} ${CONFIG}
                            fi
