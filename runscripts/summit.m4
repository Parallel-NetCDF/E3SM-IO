...
export H5VL_LOG_MASTER_SUBFILING=1
                ...
                do
                    ...
                    do
                        ...
                        do
                            ...
                            if [ "$OP" = "w" ] ; then
                                jsrun -X 1 -p ${NP} -n ${NA} -r 2 -d plane:42 -c 21 -g 0 -b packed:smt:2 -l cpu-cpu --stdio_mode collected ../src/${APP} -g ${NG} -k -o ${OUTDIR}/testfile -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p -y ${FFREQ} ${CONFIG} 
                            else
                                jsrun -X 1 -p ${NP} -n ${NA} -r 2 -d plane:42 -c 21 -g 0 -b packed:smt:2 -l cpu-cpu --stdio_mode collected ../src/${APP} -g ${NG} -k -i ${OUTDIR}/testfile_rd -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p -y ${FFREQ} ${CONFIG} 
                            fi
