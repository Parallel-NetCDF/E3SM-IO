...
#define WKL
#ifdef WKL
printf("--------------- DEBUG in %s ---  manually spit to 2 groups ---- to be removed ---\n",__FILE__);
    int orig_nprocs;
    MPI_Comm_size(orig_comm, &orig_nprocs);
    color = (orig_rank < orig_nprocs/2) ? 0 : 1;
    err = MPI_Comm_split(orig_comm, color, orig_rank, sub_comm);
