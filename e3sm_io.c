...
int main (int argc, char **argv) {
    ...
    int i, rank, nprocs, err, nerrs = 0, tst_vard = 0, tst_varn = 0, tst_wr = 0, tst_rd = 0,
                              noncontig_buf = 0, api = E3SM_API_CLASSIC;
    ...
    while ((i = getopt (argc, argv, "hkvdnmto:r:WRs:i:a:")) != EOF) switch (i) {
            case 'v':
                verbose = 1;
                break;
            case 'k':
                ...
                break;
            case 'a':
                if (!strcmp (optarg, "classic")) {
                    api = E3SM_API_CLASSIC;
                }
#ifdef HAS_NC4
                else if (!strcmp (optarg, "nc4")) {
                    api = E3SM_API_NC4;
                }
#endif
                else {
                    if (rank == 0) printf ("Unknown API type\n");
                    MPI_Finalize ();
                    return 1;
                }
                break;
    ...
    if (tst_vard == 0 && tst_varn == 0) /* neither command-line option -d or -n is used, run both */
        tst_vard = tst_varn = 1;

    if (tst_wr == 0 && tst_rd == 0) /* neither command-line option -R or -W is used, run write */
        tst_wr = 1;

    /* set the output folder name */
    if (tst_wr) {
        if (out_dir[0] == '\0') { strcpy (out_dir, "."); }
        if (verbose && rank == 0) printf ("output folder name =%s\n", out_dir);
    }

    /* set the input folder name */
    if (tst_rd) {
        if (in_dir[0] == '\0') { strcpy (in_dir, "."); }
        if (verbose && rank == 0) printf ("input folder name =%s\n", in_dir);
    }

    if (io_stride < 1) io_stride = 1;
    ...
    if (num_decomp == 3)
        run_f_case = 1;
    else if (num_decomp == 6)
        run_g_case = 1;

    if (api == E3SM_API_CLASSIC || api == E3SM_API_NC4) {
        ...
        if (run_f_case) {
            ...
            if (tst_vard) {
                ...
                if (!rank)
                    printf (
                        "PnetCDF vard API requires internal and external data types match, skip\n");
#else
                if (api == E3SM_API_NC4) {
                    if (!rank) printf ("NC4 not supported for vard\n");
                } else {
                    if (tst_rd) {
                        if (!rank) printf ("Reading not supported for vard\n");
                    }
            ...
            if (tst_varn) {
                ...
                if (tst_rd) {
                    ...
                    nerrs += run_varn_F_case_rd (io_comm, in_dir, outfname, cmode, nvars, num_recs,
                                                 noncontig_buf, info, dims, contig_nreqs, disps,
                ...
                if (tst_wr) {
                    ...
                    outfname = "f_case_h0_varn.nc";
                    nerrs +=
                        run_varn_F_case (io_comm, out_dir, outfname, cmode, nvars, num_recs,
                                         noncontig_buf, info, dims, contig_nreqs, disps, blocklens,
                                         dbl_buf_h0, rec_buf_h0, txt_buf[0], int_buf[0]);
                    ...
                    nerrs +=
                        run_varn_F_case (io_comm, out_dir, outfname, cmode, nvars, num_recs,
                                         noncontig_buf, info, dims, contig_nreqs, disps, blocklens,
                                         dbl_buf_h1, rec_buf_h1, txt_buf[1], int_buf[1]);
                }
            }
            for (i = 0; i < 3; i++) {
                free (disps[i]);
            ...
            if (dbl_buf_h0 != NULL) { free (dbl_buf_h0); }
            if (dbl_buf_h1 != NULL) { free (dbl_buf_h1); }
            if (rec_buf_h0 != NULL) { free (rec_buf_h0); }
            if (rec_buf_h1 != NULL) { free (rec_buf_h1); }
        }

        if (run_g_case) {
            if (verbose && rank == 0) {
                printf ("number of requests for D1=%d D2=%d D3=%d D4=%d D5=%d D6=%d\n",
                        contig_nreqs[0], contig_nreqs[1], contig_nreqs[2], contig_nreqs[3],
                        contig_nreqs[4], contig_nreqs[5]);
            }
            ...
            if (tst_varn) {
                ...
                if (tst_rd) {
                    ...
                    nerrs += run_varn_G_case_rd (
                        io_comm, in_dir, outfname, cmode, nvars, num_recs, info, dims, contig_nreqs,
                ...
                if (tst_wr) {
                    ...
                    nvars    = 52;
                    outfname = "g_case_hist_varn.nc";
                    nerrs += run_varn_G_case (io_comm, out_dir, outfname, cmode, nvars, num_recs,
                                              info, dims, contig_nreqs, disps, blocklens,
                                              D1_fix_int_buf, D2_fix_int_buf, D3_fix_int_buf,
                                              D4_fix_int_buf, D5_fix_int_buf, D1_rec_dbl_buf,
                                              D3_rec_dbl_buf, D4_rec_dbl_buf, D5_rec_dbl_buf,
                                              D6_rec_dbl_buf, D1_fix_dbl_buf);
                }

                if (D1_rec_dbl_buf != NULL) { free (D1_rec_dbl_buf); }
                if (D3_rec_dbl_buf != NULL) { free (D3_rec_dbl_buf); }
                if (D4_rec_dbl_buf != NULL) { free (D4_rec_dbl_buf); }
                if (D5_rec_dbl_buf != NULL) { free (D5_rec_dbl_buf); }
                if (D6_rec_dbl_buf != NULL) { free (D6_rec_dbl_buf); }
                if (D1_fix_dbl_buf != NULL) { free (D1_fix_dbl_buf); }
                if (D1_fix_int_buf != NULL) { free (D1_fix_int_buf); }
                if (D2_fix_int_buf != NULL) { free (D2_fix_int_buf); }
                if (D3_fix_int_buf != NULL) { free (D3_fix_int_buf); }
                if (D4_fix_int_buf != NULL) { free (D4_fix_int_buf); }
                if (D5_fix_int_buf != NULL) { free (D5_fix_int_buf); }
            }
            for (i = 0; i < 6; i++) {
                free (disps[i]);
                free (blocklens[i]);
...
fn_exit:
    ...
    if (io_comm != MPI_COMM_WORLD && io_comm != MPI_COMM_NULL) MPI_Comm_free (&io_comm);
