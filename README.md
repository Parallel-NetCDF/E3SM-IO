...
PIO as the `decomposition file`. This benchmark currently contains two cases,
namely F and G, from E3SM. The F case uses three unique data decomposition
patterns shared by its 381 2D and 3D variables. Th G case uses 6 data
decompositions shared by its 51 variables.
...
* Prepare the data decomposition file
  * For the F case, the three data decomposition files generated by PIO library
    ...
    parallel as the input file by this benchmark program. Similarly, the six
    decomposition files need to be converted first.
