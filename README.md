...
* Example run commands using `mpiexec` and 16 MPI processes:
  ...
  + Run read the test using HDF5 API with rearranger in the E3SM benchmark.
    ```
      ...
      % mpiexec -n 16 ./e3sm_io -a hdf5_ra -i ${Input_File_Path} datasets/f_case_866x72_16p.nc
  + Run read the test with PnetCDF and use the data read to run write test with ADIOS2.
    ```
      ...
      % mpiexec -n 16 ./e3sm_io -a adios -i ${Input_File_Path} -o ${Output_File_Path} datasets/f_case_866x72_16p.nc
    ```
  ...
  ```
    ...
    Usage: ./src/e3sm_io [OPTION]... FILE
       ...
       [-i path] Enable read performance evaluation and set the input file
              ...
              The input files must be generated by E3SM under the same configuration (decomposition file) as the selected test case.
              The API used in the read test is determined by the input file format.
              The API selected (-a option) acts only as a hint to select proper VOL when reading HDF5 files.
              When testing F case, the input files' name must be postfixed with "_h0" and "_h0" the same way as the output file.
       [-o path] Enable write performance evaluation and set the output file
                 (folder) path
              The file extension is determined based on the API used and should not be part of "path".
              ...
              If read test is also selected (-i option), the benchmark will perform read test first and use the data read in the write test.
              If read test is not selected, the benchmark will write out random data in the write test.
