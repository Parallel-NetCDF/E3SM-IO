...
* Example run commands using `mpiexec` and 16 MPI processes:
  + RUn the test with the default settings.
    ```
      % mpiexec -n 16 ./e3sm_io datasets/f_case_866x72_16p.nc
    ```
  + Assigning the output file name.
    ```
      % mpiexec -n 16 ./e3sm_io -o ${Output_File_Name} datasets/f_case_866x72_16p.nc
    ```
  + Run the test with real data collected in an E3SM simulation.
    ```
      % ln -s ${Path_to_E3SM_F_Case_H0_Output_File} ${Input_File_Name}_h0.nc
      % ln -s ${Path_to_E3SM_F_Case_H1_Output_File} ${Input_File_Name}_h1.nc
      % mpiexec -n 16 ./e3sm_io -i ${Input_File_Name} datasets/f_case_866x72_16p.nc
    ```
...
* Command-line options:
  ```
    ...
    Usage: ./e3sm_io [OPTION]... FILE
       ...
       [-o output_file_name] Output file name (default ./e3sm_io_test_file)
              The F case test output 2 files. The E3SM benchmark will append "_h0" and "_h0" to them accordingly (see "Output files" section in the README file).
       [-i intput_file_name] Path to the input files
              If set, E3SM I/O will read the data from the input files and write it out in the write test.
              The input files must be generated by E3SM under the same configuration (decomposition file) as the test case and be written by the same underlying I/O library.
              When testing F case, the input files' name must be postfixd with "_h0" and "_h0" the same way as the output file.
...
* The above example command uses command-line option `-k` to keep the output
  ...
  For the F case, each run of `e3sm_io` produces two output files named
  `f_case_h0`, and `f_case_h1`.
  ...
  + If the user specify the file name with "-o" option, the E3SM benchmark will add 
  `_h0` and `_h1` postfix to the output file names respectively.
* For the G case, there is one output file, namely `g_case`.
...
* The output file name extension is determined by the underlying API used.
  + When using PnetCDF, the extension is `.nc`.
  + When using HDF5, the extension is `.h5`.
  + When using ADIOS2, the extension is `.bp.dir`.
